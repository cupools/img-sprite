...
fn.buildImage = function(tag, imgFiles) {
    ...
    spritesmith({
        ...
        var basename = options.prevTag + tag + '.png';
        ...
        var output = path.join(options.destImg, basename.replace(/(\.png)/, '@2x$1'));
        ...
        fs.writeFile(output, result.image, 'binary', function(err) {
            ...
            if (that.options.retina) {
                new Jimp(output, function(err) {
                    if (err) {
                        throw err;
                    }
                    var imgPath = output.replace('@2x', '');
                    this.resize(ceil(properties.width / pow), ceil(properties.height / pow))
                        .write(imgPath);
...
fn.buildCss = function() {
    ...
    this.options.src.forEach(function(filepath) {
        ...
        if (that.options.retina) {
            var media = {
                'type': 'media',
                'media': that.options.media || 'only screen and (min-device-width : 768px) and (max-device-width:1024px) and (orientation : landscape)and (-webkit-min-device-pixel-ratio: 2)',
                'rules': []
            };
